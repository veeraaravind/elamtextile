...
class User extends BaseModel implements \yii\web\IdentityInterface
{
    ...
    public function afterSave($insert, $changedAttributes)
    {
        ...
        if (array_key_exists('MapWeaverLoom', $postData)) {
            foreach ($postData['MapWeaverLoom'] as $index => $eachLoom) {
                $model = new MapWeaverLoom;
                if (!empty($eachLoom['id']) && ($tempModel = MapWeaverLoom::findOne($eachLoom['id']))) {
                    $model = $tempModel;
                }
                $eachLoom['weaver_id'] = $this->id;
                if (!$model->load(['MapWeaverLoom'=>$eachLoom]) || !$model->save()) {
                    throw new Exception(
                        sprintf(
                            '%s {%s} [%s]',
                            Yii::t('app', 'Error in saving data'),
                            $eachLoom['loom_name'],
